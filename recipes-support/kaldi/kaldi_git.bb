...
EXTRA_OECMAKE += "--static --use-cuda=no --mathlib=CLAPACK --clapack-root=${LINALG_INSTALL_DIR}/packages/ti/linalg --fst-root=${STAGING_DIR_TARGET}${prefix} --fst-version=1.6.2 --ti-cross-compile --ti-device=AM57"

do_configure() {
	cd src
	chmod +x configure
	./configure ${EXTRA_OECMAKE}
}

do_compile() {
	cd src
	make online2bin featbin
}
